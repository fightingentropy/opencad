...
import { renderSupport3D, createSupportClashTester } from './SupportRender3D';
...
import { setContainmentCoversOpen } from './ContainmentRender3D';
import { createInstallationAppearance, entitySceneRoots, sceneEntities, type InstallationAppearance, type InstallationFilter } from './InstallationAppearance';
...
export interface SceneControls {
  ...
  setInstallation(project: Project, mode: InstallationAppearance, status: InstallationFilter): void;
  setLayerVisible(layer: SceneLayer, visible: boolean): void;
  isolateEntity(entityId: string | null): void;
  setExploded(distance: number): void;
  setPanelsOpen(open: boolean): void;
  setCoversOpen(open: boolean): void;
...
function supportRotationForContainment(
  ...
  for (let i = 0; i < containment.points.length - 1; i++) {
    ...
    const cx = ax + dx * t;
    const cy = ay + dy * t;
    const dz = support.elevation != null && a.z != null && b.z != null ? support.elevation - (a.z + (b.z - a.z) * t) : 0;
    const distSq = (px - cx) ** 2 + (py - cy) ** 2 + dz ** 2;
...
function connectedContainmentAt(
  position: FittingEntity['position'] & { z?: number },
  parentId: string,
  containments: ContainmentEntity[],
): boolean {
...
function shouldSkipFitting3D(
  ...
  const isConnectedEnd =
    isEndCap &&
    connectedContainmentAt({ ...fitting.position, z: fitting.elevation }, parent.id, containments);
...
function wallOpeningsForSegment(
  ...
  for (const run of runs) {
    for (let i = 0; i < run.points.length - 1; i++) {
      ...
      const baseZ = (a.z ?? run.baseZ) + ((b.z ?? run.baseZ) - (a.z ?? run.baseZ)) * hit.s;
      spans.push({
        left: center - holeW / 2,
        right: center + holeW / 2,
        zBottom: baseZ - WALL_OPENING_CLEARANCE,
        zTop: baseZ + run.height + WALL_OPENING_CLEARANCE,
      });
    }
  }
...
function buildWallGroup(
  ...
  for (const w of walls) {
    ...
    for (let i = 0; i < w.points.length - 1; i++) {
      ...
      const openings = w.structuralRole || (w.elevation ?? 0) !== 0 ? [] : wallOpeningsForSegment(
        a.x,
        ay,
        b.x,
        by,
...
function renderFloor(
  ...
  if (wantWalls || options.containmentOnly && walls.some(w => w.structuralRole)) {
    grp.add(buildWallGroup(
      options.containmentOnly ? walls.filter(w => w.structuralRole) : walls,
  ...
  if (wantContainment) {
    ...
    for (const c of renderContainments) {
      if (c.containmentType === 'conduit' && !Number.isFinite(c.elevation)) continue;
  ...
  if (wantContainment && layers.cables !== false) {
    ...
    for (const c of renderContainments) {
      const routed = cables.filter((cable) => cable.route?.includes(c.id));
      if (!routed.length) continue;
      const bundle = renderCablesInContainment(c, routed, { floor, flipY: options.flipY, maxCables: 24 });
      bundle.userData.systemId = resolveSystemId(c, options);
      cableGroup.add(bundle);
    }
    grp.add(cableGroup);
  }
  ...
  if (wantFittings) {
    ...
    for (const f of visibleFittings) {
      ...
      const baseZ = parent
        ? (f.elevation ?? defaultElevation(parent, floor)) + (parent.height ?? 50) / 2
        : 2400;
  ...
  if (wantSupports) {
    ...
    for (const s of visibleSupports) {
      ...
      if (cid) {
        ...
        if (c) {
          parentContainment = c;
          bottomZ = s.elevation ?? defaultElevation(c, floor);
        }
      }
      ...
      const hideAutoRouteRods = false;
      const obj = renderSupport3D(renderSupport, {
        containmentBottomZ: bottomZ ?? s.elevation ?? 2400,
    ...
    const clashes = createSupportClashTester(grp.getObjectByName('containment')?.children ?? []);
    const supportEntities = new Map(visibleSupports.map((support) => [support.id, support]));
    for (const support of sgrp.children) {
      if (supportEntities.get(support.userData.entityId)?.autoGenerated && clashes(support)) {
        support.userData.excludedFrom3D = 'Autogenerated hanger requires coordination at a route junction';
  ...
  if (wantEquipment) {
    ...
    for (const eq of visibleEquipment) {
      const equipmentObject = renderEquipment3D(eq, { flipY: options.flipY });
      equipmentObject.userData.systemId = resolveSystemId(eq, options);
      egrp.add(equipmentObject);
    }
    grp.add(egrp);
  }
  ...
  if (layers.firestops !== false) {
    ...
    for (const sheetId of floor.sheetIds) {
      ...
      for (const entity of Object.values(sheet.entities)) {
        if (entity.kind !== 'penetration' || entity.visible === false) continue;
        seals.add(renderPenetration3D(entity, project, floor, options.flipY));
      }
    }
    grp.add(seals);
  }
...
export function buildBuildingScene(
  ...
  const applyAppearance = createInstallationAppearance(root);
  let currentProject = project;
  let currentSystem: SystemId | null = null;
  let currentStatus: InstallationFilter = 'all';
  let isolatedEntity: string | null = null;
  const entityRoots = entitySceneRoots(root);
  const layerVisibility = new Map<SceneLayer, boolean>();
  const updateVisibility = (): void => {
    const entities = sceneEntities(currentProject);
    for (const obj of entityRoots) {
      const entity = entities.get(obj.userData.entityId);
      if (!entity || ['wall', 'room'].includes(entity.kind)) continue;
      const sid = entity.systemId ?? obj.userData.systemId;
      obj.visible = !obj.userData.excludedFrom3D && entity.visible !== false
        && (currentSystem == null || sid === currentSystem)
        && (currentStatus === 'all' || installationStatus(entity) === currentStatus)
        && (isolatedEntity == null || entity.id === isolatedEntity);
    }
    for (const info of floorInfos) {
      info.group.traverse((obj) => {
        const layer = (obj.userData.layer ?? obj.name) as SceneLayer;
        if (['walls', 'rooms', 'floors', 'labels', 'containment', 'equipment', 'fittings', 'supports', 'risers', 'cables', 'firestops'].includes(layer)) {
          obj.visible = (layerVisibility.get(layer) ?? true)
            && !(isolatedEntity && ['walls', 'rooms', 'floors', 'labels'].includes(layer));
        }
      });
    }
  };
  ...
  const setInstallation = (next: Project, mode: InstallationAppearance, status: InstallationFilter): void => {
    currentProject = next;
    currentStatus = status;
    applyAppearance(next, mode);
    updateVisibility();
  };
  const setLayerVisible = (layer: SceneLayer, visible: boolean): void => {
    layerVisibility.set(layer, visible);
    updateVisibility();
  };
  const isolateEntity = (entityId: string | null): void => {
    isolatedEntity = entityId;
    updateVisibility();
  };
  const setExploded = (distance: number): void => {
    const gap = Number.isFinite(distance) ? Math.max(0, Math.min(distance, 10000)) : 0;
    for (const info of floorInfos) info.group.position.z = info.floor.ffl + Math.max(0, info.floor.level) * gap;
    root.updateMatrixWorld(true);
  };
  const setPanelsOpen = (open: boolean): void => setEquipmentOpen(root, open);
  const setCoversOpen = (open: boolean): void => setContainmentCoversOpen(root, open);
  ...
  return {
    ...
    controls: {
      ...
      setInstallation, setLayerVisible, isolateEntity, setExploded, setPanelsOpen, setCoversOpen,
      dispose,
    },
  };
}
