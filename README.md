...
An **Add** result starts a pointer preview. Choose a route's length and elevation, rotate with **R**, then click to place. Enable **Repeat** to keep placing the same part; Escape ends the preview. Containment, equipment and supports stay in the simple 3D workspace. Schematic symbols use 2D; underground ducts also use 2D in a full site scene where they are intentionally hidden. Small symbols zoom in for placement without changing their dimensions. Each placement has its own undo step and respects layer and collaboration permissions. Catalogue records need sufficient dimensions to be addable; other records remain available in the catalogue browser.

Select a physical part to edit its dimensions in the sidebar. **Enter** or leaving a field commits the edit; **Escape** restores the field. Drag the centre handle to move, drag route points to reshape, or use **Move (G)**, **Rotate (R)**, **Duplicate (⌘D)** and **Extend** below the canvas. Shift constrains route points to 45° axes. Compatible routes snap at the same elevation and section size; endpoints take priority, and a branch can connect along a route. Bends, tees, crosses and couplers regenerate with the route in one undo step. Existing accessory installation records are preserved. Custom dimensions clear the original catalogue identity, and duplicates receive fresh identities without copied installation progress or cable assignments.
